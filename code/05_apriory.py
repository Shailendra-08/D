...
def apriori_algorithm(input_file, output_file, minfre):
    ...
    output = f"Minimum support count: {min_support:.2f} (calculated as {minfre}% of {len(datatable)})\n"
    ...
    while prev:
        ...
        for itemset_str in sorted(cur_freq.keys()):
            ...
            output += f"{itemset_str: <15}: {support_count} (Support calculated from {support_count} occurrences)\n"
...
if __name__ == "__main__":
    ...
    input_file = os.path.join(script_dir, '05_frequent_itemset_data.csv')
    output_file = os.path.join(script_dir, 'apriori.txt')  
    try:
